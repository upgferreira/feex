...
export const CategoryModal: React.FC<CategoryModalProps> = ({ isOpen, onClose }) => {
  ...
  const filtered = categories.filter(c =>
    [c.canal, c.grupo, c.categoria_canal, c.categoria_pai_erp, c.categoria_erp, c.tipo]
          ...
          {loading ? (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0">
                <tr>
                  {COLS.map(c => (
                    <th key={c.key} className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap">
                      {c.label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filtered.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-gray-500 dark:text-gray-400">Nenhum resultado encontrado</td></tr>
                ) : filtered.map((c, i) => (
